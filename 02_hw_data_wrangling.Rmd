...
```
There are 7014 candidates total.
...
knitr::kable(party_incumbent_promote, digits = 3, col.names = c("Party Affiliation", "Incumbency Status", "Promotional Ad Average"))
```

Incumbents from the Democrat political party sponsored the most promoting ads on average.
...
This histogram has a large spike on the left and quickly declines, and most candidates did not have more than 10 million ad impressions on facebook.
...
fb_top_plot <- fb_ads |>
  ...
  ggplot(mapping = aes(x = impressions_millions, y = reorder(cand_name_last, impressions_millions))) +
  geom_bar(stat = "identity", aes(fill = party), na.rm = T) +
  labs(title = "Facebook Ad Impressions of Top 15 US House candidates",
       x = "Candidates",
       y = "Millions of Impressions")
fb_top_plot
```
